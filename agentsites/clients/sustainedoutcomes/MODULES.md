...
| --- | --- | --- |
| Angled-edge scroll motion | **done 2026-05-11** | Shipped in `FeatureSplit.astro`. Each `.feature-split-angled` element's clip-path is driven by two CSS custom properties (`--angle-top`, `--angle-bot`) that tween 0px → 24px linearly as the section moves through `view()`-style scroll progress. Net: panel appears to open up ~48px vertically while content stays anchored. rAF-throttled scroll listener gated by `IntersectionObserver` (off-screen sections cost zero). `prefers-reduced-motion: reduce` `!important`-locks vars to 0. The v1 one-shot entry reveal was dropped — the scroll-tied motion now serves as both entry and through-scroll effects. |
